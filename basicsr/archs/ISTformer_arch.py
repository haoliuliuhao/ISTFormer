# model: ISTFormer
# SRFormer: ISTFormer: A Lightweight Transformer-based Approach for Super-Resolving Coal Rock Images through Iterative Sampling
...
from archs.hebing import GPSAB, LCBlock
from archs.iudt import *
...
class ISTFormer(nn.Module):
    r""" ISTFormer
        A PyTorch implement of : `ISTFormer: A Lightweight Transformer-based Approach for Super-Resolving Coal Rock Images through Iterative Sampling`, based on SRformer.
    ...
    def __init__(self,
        ...
        self.gpsb = GPSAB(dim=embed_dim, input_resolution=(h, w), num_heads=num_heads[0], window_size=window_size)
    ...
    # DFE
    def forward_features(self, x):  # [1, 60, 264, 184]
        x_size = (x.shape[2], x.shape[3])   # list  ->  [264, 184]
        
        x = self.patch_embed(x) # [1, 60, 264*184]  ->  [1, 48576, 60]
        if self.ape:    # 添加位置信息
            x = x + self.absolute_pos_embed # [1, 48576, 60]
        x = self.pos_drop(x)
    ...
    def forward(self, x):
        H, W = x.shape[2:]  # 264 184
        x = self.check_image_size(x)
        self.mean = self.mean.type_as(x)    # 1,3,1,1
        x = (x - self.mean) * self.img_range    # [1, 3, 264, 184]

        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.conv_first(x)  # [1, 60, 264, 184]
